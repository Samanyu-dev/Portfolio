...
interface FluidParticlesBackgroundProps {
  ...
  className?: string;
  /** When true, canvas sits behind content without forcing full-screen centering */
  fixed?: boolean;
}
...
const COLOR_SCHEME = {
  light: { background: "rgba(255, 255, 255, 0.14)" },
...
export function FluidParticlesBackground({
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      for (const particle of particles) {
        ...
        ctx.fillStyle = isDark ? `rgba(255, 255, 255, ${opacity})` : `rgba(37, 99, 235, ${opacity * 0.6})`;
        ctx.beginPath();
  ...
  return (
    <div
      className={cn(
        fixed ? "pointer-events-none fixed inset-0 -z-10" : "relative w-full overflow-hidden",
        !fixed && "min-h-screen bg-bg-0",
        className
      )}
    >
      <canvas ref={canvasRef} className="absolute inset-0 h-full w-full" />
      {children ? (
        <div className={cn("relative z-10", fixed ? "contents" : "flex min-h-screen w-full items-center justify-center")}>
          {children}
        </div>
      ) : null}
    </div>
